...
# data->原始数据(n,m)表示n个样本，m个指标 (成本型数据要取反)
# weight->各指标权重,list
# full_rank->秩和比方法，True为简单的整次秩比法
...
def rsrAnalysis(data, file_name=None, **kwargs):
    Result, Distribution = rsr(data, **kwargs) # 测评结果计算
...
if __name__=='__main__':
    data=pd.read_csv(r'final_data.csv',index_col=0) # Type 列作为索引
    rsrAnalysis(data,weight=[0.731995,0.020533,0.247472])
